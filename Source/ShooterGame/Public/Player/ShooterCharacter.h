...
protected:
	...
	//Pawn(Fort) Possession
	APawn* NewPawn;
	APlayerController* OurPlayerController;

	// UPROPERTY(BlueprintReadWrite,EditAnywhere,Category=Animation)
	// UTimelineComponent* FadeOutTimeLine;          //???????TimeLine???Fade Out
	//
	// UPROPERTY(BlueprintReadWrite,EditAnywhere,Category=Animation)
	// UCurveFloat* FadeOutValueCurve;    //????TimeLine????????
...
protected:
	...
	/** Change to DeathMaterial*/
	//UFUNCTION(BlueprintImplementableEvent)     //????????????????????TimeLine
	UFUNCTION(BlueprintImplementableEvent)
	void SetDeathMaterialAndFadeOut();
...
public:
	...
	// UPROPERTY(BlueprintReadWrite,EditAnywhere,Category=Animation)
	// UMaterialParameterCollection* FadeOutMPC;
