...
public:
	 ...
	 */
	bool ShowScoreboard(bool bEnable, bool bFocus = false);   //打开菜单后，得分面板不可打开
	...
	void SetFreeDrag(bool _IsFreeDrag);

	inline bool GetFreeDrag();

	void SetKillFreeDrag();
	void SetHealthFreeDrag();
	void SetTimeFreeDrag();
	void SetWeaponFreeDrag();

	void SetHasStartMouse(bool _HasStartMouse);

	void SetStartMouse(FVector2D Mouse);
...
protected:
	...
	/** 通过偏移来实现UI的FreeDrag*/
	FVector2D KillOffset;
	FVector2D HealthOffset;
	FVector2D TimeOffset;
	FVector2D WeaponOffset;
	...
	/** General offset for HUD elements. */   //平均偏移
	float Offset;
