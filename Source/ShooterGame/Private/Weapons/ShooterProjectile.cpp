...
void AShooterProjectile::InitGravity(float value)
{
	if(MovementComp)
	{
		MovementComp->ProjectileGravityScale=value;
	}
}
...
void AShooterProjectile::Explode(const FHitResult& Impact)
{
	...
	// effects and damage origin shouldn't be placed inside mesh at impact point  不应放置在网格内
	const FVector NudgedImpactLocation = Impact.ImpactPoint + Impact.ImpactNormal * 10.0f;

	if (WeaponConfig.ExplosionDamage > 0 && WeaponConfig.ExplosionRadius > 0 && WeaponConfig.DamageType)
	{
		UGameplayStatics::ApplyRadialDamage(this, WeaponConfig.ExplosionDamage, NudgedImpactLocation,
			WeaponConfig.ExplosionRadius, WeaponConfig.DamageType, TArray<AActor*>(), this, MyController.Get());
		// if(MyController.Get())
		// {
		// 	GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,MyController.Get()->GetName());   //Fort发射的炮弹没有Controller
		// }
	}

	if (ExplosionTemplate)   //有爆炸效果
	{
...
void AShooterProjectile::SetController(AController* Controller)
{
