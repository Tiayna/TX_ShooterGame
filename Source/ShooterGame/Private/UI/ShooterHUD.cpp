...
AShooterHUD::AShooterHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	//默认设置下没有进行FreeDrag操作时
	KillOffset=FVector2D::ZeroVector;
	HealthOffset=FVector2D::ZeroVector;
	TimeOffset=FVector2D::ZeroVector;
	WeaponOffset=FVector2D::ZeroVector;
	StartMouse=FVector2D::ZeroVector;
	CurrentOffset=nullptr;
	HasStartMouse=false;
	IsFreeDrag=false;

	//测试用
	// KillOffset = FVector2D(100.0f,100.0f);
	// HealthOffset=FVector2D(100.0f,-100.0f);
	// TimeOffset=FVector2D(-100.0f,100.0f);
	// WeaponOffset=FVector2D(-100.0f,-100.0f);
	//CurrentOffset=&KillOffset;
	...
	//通过HUDMain资产来提取出所要用到的各种Icon   BG:BackGround
...
void AShooterHUD::DrawWeaponHUD()
{
	...
	if (MyWeapon)
	{
		...
		//PRIMARY WEAPON
		{
			...
			const float PriWeapBgPosY =  Canvas->ClipY - Canvas->OrgY - (PriWeapOffsetY + PrimaryWeapBg.VL + Offset) * ScaleUI + WeaponOffset.Y;   //-100.0f：背景黑框上移

			//Weapon draw position   武器图标位置
			...
			FCanvasTileItem TileItem(FVector2D( PriClipPosX - Offset * ScaleUI + WeaponOffset.X, PriWeapBgPosY ), PrimaryWeapBg.Texture->Resource, //-100.0f：背景黑框一部分左移
			...
			Canvas->DrawItem( TileItem );   //主武器背景1

			const float RestWidth =  Canvas->ClipX - PriClipPosX - LeftCornerWidth * ScaleUI;
			TileItem.Position = FVector2D(PriClipPosX - (Offset - LeftCornerWidth) * ScaleUI + WeaponOffset.X, PriWeapBgPosY);   //Draw Position   //-100.0f：背景黑框一部分左移
			...
			Canvas->DrawItem( TileItem );  //主武器背景2

			//Drawing primary weapon icon, ammo in the clip and total spare ammo numbers  绘制主武器图标，弹夹中的弹药和总备用弹药数量
			Canvas->DrawIcon(MyWeapon->PrimaryIcon, PriWeapPosX + WeaponOffset.X, PriWeapPosY + WeaponOffset.Y, ScaleUI);   //主武器图标位置
			...
			Canvas->DrawItem( TextItem, TopTextPosX + WeaponOffset.X, TopTextPosY + WeaponOffset.Y );    //弹夹数位置
			...
			TextItem.FontRenderInfo = ShadowedFont;
			Canvas->DrawItem( TextItem, BottomTextPosX+WeaponOffset.X, BottomTextPosY+WeaponOffset.Y );    //备弹数位置
			...
			for (int32 i = 0; i < MyWeapon->AmmoIconsCount; i++)
			{
				...
				Canvas->DrawIcon(MyWeapon->PrimaryClipIcon, PriClipPosX + ClipOffset+WeaponOffset.X, PriClipPosY+WeaponOffset.Y, ScaleUI);  //弹夹图标
			}
		...
		if (SecondaryWeapon)
		{
			...
			const float SecWeapBgPosY =  Canvas->ClipY - Canvas->OrgY - (SecondaryWeapBg.VL + Offset) * ScaleUI + WeaponOffset.Y ;    //参考主武器背景黑框偏移
			...
			FCanvasTileItem TileItem(FVector2D(  SecClipPosX - Offset * ScaleUI + WeaponOffset.X, SecWeapBgPosY ), SecondaryWeapBg.Texture->Resource, 
			...
			TileItem.BlendMode = SE_BLEND_Translucent;
			Canvas->DrawItem(TileItem);    //黑框

			const float RestWidth =  Canvas->ClipX - SecClipPosX - LeftCornerWidth * ScaleUI;
			TileItem.Position = FVector2D(SecClipPosX - (Offset - LeftCornerWidth) * ScaleUI + WeaponOffset.X, SecWeapBgPosY);
			...
			for (int32 i = 0; i < SecondaryWeapon->AmmoIconsCount; i++)
			{
				...
				Canvas->DrawIcon(SecondaryWeapon->SecondaryClipIcon, SecClipPosX + ClipOffset+WeaponOffset.X, SecClipPosY+WeaponOffset.Y, ScaleUI);  //弹夹图标
			}

			//Drawing secondary weapon icon, ammo in the clip and total ammo numbers
			Canvas->SetDrawColor(FColor::White);
			Canvas->DrawIcon(SecondaryWeapon->SecondaryIcon, SecWeapPosX+WeaponOffset.X, SecWeapPosY+WeaponOffset.Y, ScaleUI);    //副武器图标
			...
			TextItem.Text = FText::FromString( Text );
			TextItem.Scale = FVector2D( TopTextScale * ScaleUI, TopTextScale * ScaleUI );
			Canvas->DrawItem( TextItem, TopTextPosX+WeaponOffset.X, TopTextPosY);   //副武器备弹数
		}
...
void AShooterHUD::DrawHealth()
{
	...
	//HealthPos血条整体偏移，可用于实现FreeDrag
	Canvas->SetDrawColor(FColor::White);
	const float HealthPosX = (Canvas->ClipX - HealthBarBg.UL * ScaleUI) / 2 + HealthOffset.X;
	const float HealthPosY = Canvas->ClipY - (Offset + HealthBarBg.VL) * ScaleUI + HealthOffset.Y;
...
void AShooterHUD::DrawMatchTimerAndPosition()
{
	...
	//TimerPos用于部分偏移，此部分代码最后还有用于“沙漏”图标和Team标识的偏移
	Canvas->SetDrawColor(FColor::White);
	const float TimerPosX = Canvas->ClipX - Canvas->OrgX - (TimePlaceBg.UL + Offset) * ScaleUI + TimeOffset.X;
	const float TimerPosY = Canvas->OrgY + Offset * ScaleUI + TimeOffset.Y;
		...
		if (MyPC && MyGameState && MatchState == EShooterMatchState::Playing)
		{
			...
			if (MyPlayerState)
			{
				...
				Canvas->DrawIcon(PlaceIcon,
					Canvas->ClipX - Canvas->OrgX - BoxWidth  - (SizeX * TextScale + PlaceIcon.UL + Offset/4) * ScaleUI + TimeOffset.X,   //偏移
					TimerPosY + (TimePlaceBg.VL - PlaceIcon.VL) / 2.0f * ScaleUI , ScaleUI);

				TextItem.Text = FText::FromString( Text);
				TextItem.Scale = FVector2D(TextScale*ScaleUI, TextScale*ScaleUI);
				TextItem.FontRenderInfo = ShadowedFont;
				Canvas->DrawItem( TextItem, Canvas->ClipX - Canvas->OrgX - (BoxWidth  + SizeX * TextScale * ScaleUI) + TimeOffset.X,   //偏移
					TimerPosY + (TimePlaceBg.VL * ScaleUI - SizeY * TextScale * ScaleUI) / 2 );
			}
		}
	}
...
void AShooterHUD::DrawKills()
{
	...
	//KillsPos:击杀数UI整体偏移 ， 考虑在此添加offset实现freedrag
	Canvas->SetDrawColor(FColor::White);
	float KillsPosX = Canvas->OrgX + Offset * ScaleUI + KillOffset.X;
	float KillsPosY = Canvas->OrgY + Offset * ScaleUI + KillOffset.Y;
...
void AShooterHUD::DrawHUD()
{
	...
	if(HasStartMouse&&(CurrentOffset!=nullptr))
	{
		AShooterPlayerController* Controller = Cast<AShooterPlayerController>(PlayerOwner);
		if(Controller)
		{
			FVector2D CurrentMouse;
			Controller->GetMousePosition(CurrentMouse.X,CurrentMouse.Y);
			*CurrentOffset=CurrentMouse-StartMouse;
			//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,TEXT("SetOffset"));
		}
	}
	...
	AShooterCharacter* MyPawn = Cast<AShooterCharacter>(GetOwningPawn());
	if (MyPawn && MyPawn->IsAlive() && MyPawn->Health < MyPawn->GetMaxHealth() * MyPawn->GetLowHealthPercentage())     //低生命值状态
	{
	...
	if (MatchState == EShooterMatchState::Playing)
	{
		...
		DrawDeathMessages(); //死亡信息
		DrawCrosshair();     //十字准星
		DrawHitIndicator();  //受击指示器
...
void AShooterHUD::SetFreeDrag(bool _IsFreeDrag)
{
	IsFreeDrag=_IsFreeDrag;
}

bool AShooterHUD::GetFreeDrag()
{
	return IsFreeDrag;
}

void AShooterHUD::SetKillFreeDrag()
{
	CurrentOffset=&KillOffset;
}

void AShooterHUD::SetHealthFreeDrag()
{
	CurrentOffset=&HealthOffset;
}

void AShooterHUD::SetTimeFreeDrag()
{
	CurrentOffset=&TimeOffset;
}

void AShooterHUD::SetWeaponFreeDrag()
{
	CurrentOffset=&WeaponOffset;
}

void AShooterHUD::SetHasStartMouse(bool _HasStartMouse)
{
	HasStartMouse=_HasStartMouse;
}

void AShooterHUD::SetStartMouse(FVector2D Mouse)
{
	StartMouse=Mouse;
}
