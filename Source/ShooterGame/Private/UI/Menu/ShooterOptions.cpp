...
void FShooterOptions::Construct(ULocalPlayer* InPlayerOwner)
{
	...
	/** FreeDrag menu root item*/
	TSharedPtr<FShooterMenuItem> FreeDragRoot=FShooterMenuItem::CreateRoot();
	...
	FreeDragItem=MenuHelper::AddMenuItem(FreeDragRoot,LOCTEXT("Free Drag","FREE DRAG"));    //在InGameMenu里添加FREEDRAG选项
	MenuHelper::AddMenuItemSP(FreeDragItem,LOCTEXT("No","NO"),this,&FShooterOptions::OnCancelFreeDragMode);
	MenuHelper::AddMenuItemSP(FreeDragItem,LOCTEXT("Kill","KILL"),this,&FShooterOptions::KillFreeDrag);
	MenuHelper::AddMenuItemSP(FreeDragItem,LOCTEXT("Health","HEALTH"),this,&FShooterOptions::HealthFreeDrag);
	MenuHelper::AddMenuItemSP(FreeDragItem,LOCTEXT("Time","TIME"),this,&FShooterOptions::TimeFreeDrag);
	MenuHelper::AddMenuItemSP(FreeDragItem,LOCTEXT("Weapon","WEAPON"),this,&FShooterOptions::WeaponFreeDrag);
...
void FShooterOptions::OnCancelFreeDragMode()
{
	OnApplyChanges.ExecuteIfBound();   //借助用来close submenu
}

//现在就要解决需求：实现HUD的自由拖动的函数
void FShooterOptions::OnConfirmFreeDragMode(int32 type)    //设置当前ShooterHUD的FreeDrag为true，进入FreeDragMode
{
	if (ensure(PlayerOwner != nullptr))
	{
		APlayerController* BaseController = Cast<APlayerController>(UGameplayStatics::GetPlayerControllerFromID(PlayerOwner->GetWorld(), PlayerOwner->GetControllerId()));
		...
		if (BaseController)
		{
			...
			if (ShooterPlayerController)
			{
				//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,TEXT("IsFreeDrag = true"));
				
				ShooterPlayerController->GetShooterHUD()->SetFreeDrag(true);
				if(type==0)
				{
					ShooterPlayerController->GetShooterHUD()->SetKillFreeDrag();
				}
				else if(type==1)
				{
					ShooterPlayerController->GetShooterHUD()->SetHealthFreeDrag();
				}
				else if(type==2)
				{
					ShooterPlayerController->GetShooterHUD()->SetTimeFreeDrag();
				}
				else if(type==3)
				{
					ShooterPlayerController->GetShooterHUD()->SetWeaponFreeDrag();
				}
			}
			else
			{
...
void FShooterOptions::KillFreeDrag()
{
	this->Type=Kill;
	OnConfirmFreeDragMode(Type);
}

void FShooterOptions::HealthFreeDrag()
{
	this->Type=Health;
	OnConfirmFreeDragMode(Type);
}

void FShooterOptions::TimeFreeDrag()
{
	this->Type=Time;
	OnConfirmFreeDragMode(Type);
}

void FShooterOptions::WeaponFreeDrag()
{
	this->Type=Weapon;
	OnConfirmFreeDragMode(Type);
}
