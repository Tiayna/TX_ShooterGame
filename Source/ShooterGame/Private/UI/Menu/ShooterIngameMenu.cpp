...
void FShooterIngameMenu::Construct(ULocalPlayer* _PlayerOwner)
{
	...
	if (!GameMenuWidget.IsValid())
	{
		...
		MenuHelper::AddExistingMenuItem(RootMenuItem, ShooterOptions->FreeDragItem.ToSharedRef());
...
void FShooterIngameMenu::DetachGameMenu()      //退出菜单页面
{
	if (GEngine && GEngine->GameViewport)
	{
...
void FShooterIngameMenu::ToggleGameMenu()
{
	...
	if (bIsGameMenuUp && GameMenuWidget->CurrentMenu != RootMenuItem->SubMenu)   //回到上一个菜单页面
	{
		GameMenuWidget->MenuGoBack();
		return;
	}
	...
	if (!bIsGameMenuUp)  //若菜单未打开，则打开菜单
	{
		...
		GameMenuWidget->BuildAndShowMenu();   //显示出widget
		bIsGameMenuUp = true;

		if (PCOwner)
		{
			// Disable controls while paused    调出菜单时禁止控制（进入电影模式？），游戏暂停
			PCOwner->SetCinematicMode(true, false, false, true, true);

			//修改原因：为了实现作业要求4里的freedrag功能，一是如果要保证原有的暂停状态的话，由于世界的暂停，HUD和Input控制等无法实现，只能
			//在退出菜单面板后才能进行freedrag；而如果要在菜单打开的同时能够进行freedrag，不得不通过取消菜单调用时的全局暂停来实现freedrag
			//（虽然可能也有通过修改源码的途径来实现世界暂停时的freedrag，但个人能力实在有限）
	...
	else
	{
		...
		GameMenuWidget->HideMenu();  //隐藏菜单，返回到游戏中
		if (PCOwner)
		{
			// Make sure viewport has focus
			FSlateApplication::Get().SetAllUserFocusToGameViewport();    //聚焦到游戏视角
