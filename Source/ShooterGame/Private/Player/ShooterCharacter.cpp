	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<UShooterCharacterMovement>(ACharacter::CharacterMovementComponentName))
{
	...
	//????MPC
	// static ConstructorHelpers::FObjectFinder<UMaterialParameterCollection> MPC(TEXT("/Game/Characters/Materials/Material_FadeOut/MPC_FadeOut"));
	// if(MPC.Succeeded())
	// {
	// 	GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,FString::Printf(TEXT("Find MPC")));
	// 	FadeOutMPC=MPC.Object;
	// }
	//????Timeline????????
	// static ConstructorHelpers::FObjectFinder <UCurveFloat> Curve(TEXT("/Game/Characters/Materials/Material_FadeOut/FadeOutValue"));
	// if(Curve.Succeeded())
	// {
	// 	GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,FString::Printf(TEXT("Find Curve")));
	// 	FadeOutValueCurve=Curve.Object;
	// }
...
void AShooterCharacter::UpdatePawnMeshes()
{
	...
	Mesh1P->VisibilityBasedAnimTickOption = !bFirstPerson ? EVisibilityBasedAnimTickOption::OnlyTickPoseWhenRendered 
		: EVisibilityBasedAnimTickOption::AlwaysTickPoseAndRefreshBones;
	Mesh1P->SetOwnerNoSee(!bFirstPerson);

	GetMesh()->VisibilityBasedAnimTickOption = bFirstPerson ? EVisibilityBasedAnimTickOption::OnlyTickPoseWhenRendered 
		: EVisibilityBasedAnimTickOption::AlwaysTickPoseAndRefreshBones;
...
float AShooterCharacter::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, class AActor* DamageCauser)
{
	...
	Damage = Game ? Game->ModifyDamage(Damage, this, DamageEvent, EventInstigator, DamageCauser) : 0.f;   //EventInstigator»÷É±À´Ô´
...
bool AShooterCharacter::Die(float KillingDamage, FDamageEvent const& DamageEvent, AController* Killer, AActor* DamageCauser)
{
	...
	GetWorldTimerManager().SetTimer(TimerHandle, this, &AShooterCharacter::SetDeathMaterialAndFadeOut, 1.0f, false);
...
void AShooterCharacter::OnDeath(float KillingDamage, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser)
{
	...
	if (DeathAnimDuration > 0.f)
	{
		...
		const float TriggerRagdollTime = DeathAnimDuration - 0.7f;     //?????????0.5s??????????????????????0.5s???????????
		//const float TriggerFadeOutTime = DeathAnimDuration - 0.2f;     //?????????1.0s??????????????????????1.0s???§Ý??FadeOut????
...
// void AShooterCharacter::SetDeathMaterial()
// {
// 	//?§Ý?????
// 	Mesh1P->SetMaterial(0,DeathMaterial);
// 	GetMesh()->SetMaterial(0,DeathMaterial);
// 	bFadeOut=true;
// 	//TimeLine     ??????UMaterialParametersCollection?????C++??Set?
// 	// FOnTimelineFloat onTimelineCallback;
// 	// FadeOutTimeLine=NewObject<UTimelineComponent>(this,FName("TimelineAnimation"));
// 	// onTimelineCallback.BindUFunction(this,FName(TEXT("TimelineCallback")));
// 	// FadeOutTimeLine->AddInterpFloat(FadeOutValueCurve,onTimelineCallback);
// 	// FadeOutTimeLine->RegisterComponent();
// 	// FadeOutTimeLine->TickComponent(GetWorld()->GetDeltaSeconds(),ELevelTick::LEVELTICK_TimeOnly, NULL);
...
float AShooterCharacter::PlayAnimMontage(class UAnimMontage* AnimMontage, float InPlayRate, FName StartSectionName)
{
	...
	{
		return UseMesh->AnimScriptInstance->Montage_Play(AnimMontage, InPlayRate);    //????????ÑÄ?????????
	}
...
void AShooterCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("TogglePerspective", IE_Pressed, this, &AShooterCharacter::TogglePerspective);

	PlayerInputComponent->BindAction("PossessFort",IE_Pressed,this,&AShooterCharacter::PossessIt);
}
...
{
	return IsAlive() && Controller && Controller->IsLocalPlayerController() && IsFPP;
}
...
void AShooterCharacter::TogglePerspective()
{
	...
	K2_TogglePerspectiveCamera(IsFPP);
}

void AShooterCharacter::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor,
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& Hit)
{
	if(GEngine)
	{
		//DrawDebugString(GetWorld(),OtherActor->GetActorLocation(),"Press F to Take Control",nullptr,FColor::Green,5.0f,true);
		//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,FString::Printf(TEXT("Character Overlap happen")));
		NewPawn=Cast<APawn>(OtherActor);
		//AFort* Fort=Cast<AFort>(NewPawn);
		if(NewPawn)
		{
			//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,FString::Printf(TEXT("Catch Fort")));
			//Fort->OnFire();
		}
...
void AShooterCharacter::OnOverlapEnd(UPrimitiveComponent* OverlappedComp, AActor* OtherActor,
                                     UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	NewPawn=nullptr;
	if(GEngine)
	{
		//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,FString::Printf(TEXT("Left Fort")));
		NewPawn=nullptr;
	}
}

void AShooterCharacter::PossessIt()
{
	...
	MyController->SetPossessFort(true);
	
	//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,UGameplayStatics::GetPlayerController(this,0)->Player->GetName());
	if(NewPawn)
	{
		//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,TEXT("Possess Fort"));
