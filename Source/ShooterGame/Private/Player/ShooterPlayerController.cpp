...
void AShooterPlayerController::SetupInputComponent()
{
	...
	if(!bHasInitializedInputComponent)
	{
		...
		InputComponent->BindAction("FreeDragMode",IE_Pressed,this,&AShooterPlayerController::EnterFreeDragMode);
...
void AShooterPlayerController::OnToggleInGameMenu()
{
	...
	{
		AShooterPlayerController* Controller = Cast<AShooterPlayerController>(*It);    //遍历游戏世界中所有的Controller，检查暂停状态
...
void AShooterPlayerController::SetPossessFort(bool Possess)
{
	IsPossessFort=Possess;
}
...
bool AShooterPlayerController::HasPossessFort()
{
	return IsPossessFort;
}
...
void AShooterPlayerController::EnterFreeDragMode()
{
	if(GetShooterHUD()->GetFreeDrag())
	{
		if(bNextIsEnd)
		{
			GetShooterHUD()->SetFreeDrag(false);
			GetShooterHUD()->SetHasStartMouse(false);
			bNextIsEnd=false;
		}
		else
		{
			//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,TEXT("Enter Free Drag Mode"));
			FVector2D StartMouse;
			this->GetMousePosition(StartMouse.X,StartMouse.Y);
			GetShooterHUD()->SetStartMouse(StartMouse);
			GetShooterHUD()->SetHasStartMouse(true);
			bNextIsEnd=true;
		}
	}
}
