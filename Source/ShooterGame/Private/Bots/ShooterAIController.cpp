...
void AShooterAIController::OnPossess(APawn* InPawn)
{
	...
	if (Bot && Bot->BotBehavior)
	{
		...
		{
			// FGameplayTag SubTag;
			// BehaviorComp->SetDynamicSubtree(SubTag,Bot->GetSmartObject()->SubTree);
			CanRespawn=!Bot->IsbPatrol();    //巡逻类型的Bot不会重生
		}
		...
		BlackboardComp->SetValue<UBlackboardKeyType_Bool>(FName("SenseEnemy"),false);   //初始未感知到Enemy
		
		LocationKeyId=BlackboardComp->GetKeyID("MoveToLocation");
...
{
	//AShooterBot* MyBot = Cast<AShooterBot>(GetPawn());    //转换失败
	// if(!MyBot->GetbPatrol())  //巡逻类型的Bot不会重生
	// {
...
void AShooterAIController::FindClosestEnemy()
{
	...
	for (AShooterCharacter* TestPawn : TActorRange<AShooterCharacter>(GetWorld()))   //遍历世界中的其他玩家，索敌最近的
	{
...
bool AShooterAIController::FindClosestEnemyWithLOS(AShooterCharacter* ExcludeEnemy)
{
	...
	if (MyBot != NULL)
	{
		...
		for (AShooterCharacter* TestPawn : TActorRange<AShooterCharacter>(GetWorld()))
		{
			...
			{
				if (HasWeaponLOSToEnemy(TestPawn, true) == true)   //有Line Of Sight（看到就开枪）
				{
					const float DistSq = (TestPawn->GetActorLocation() - MyLoc).SizeSquared();
					if (DistSq < BestDistSq)
					{
