...
void AShooterGameMode::Killed(AController* Killer, AController* KilledPlayer, APawn* KilledPawn, const UDamageType* DamageType)
{
	...
	AShooterPlayerController* MyController=Cast<AShooterPlayerController>(Killer);
	if(MyController)
	{
		bFort=MyController->HasPossessFort();	
	}
	//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,Killer->GetName());
	
	if (KillerPlayerState && KillerPlayerState != VictimPlayerState)     //击杀者获得击杀数
	{
		//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,FString::Printf(TEXT("Kill Get Point")));
		if(bFort)
		{
			KillerPlayerState->ScoreKill(VictimPlayerState,2*KillScore);
		}
		else
		{
	...
	if (VictimPlayerState)                                               //被击杀者获得死亡数
	{
		//GEngine->AddOnScreenDebugMessage(-1,5.0f,FColor::Yellow,FString::Printf(TEXT("Death Lost Point")));
